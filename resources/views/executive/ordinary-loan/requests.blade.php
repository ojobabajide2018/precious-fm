                ...
                <div class="row page-titles mx-0">
                    <div class="col-sm-6 p-md-0">
                        <div class="welcome-text">
                            <h4 class="text-primary">Ordinary Loan Requests</h4>
                        </div>
                    </div>
                    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="">Home</a></li>
                            <li class="breadcrumb-item active"><a href="">Member's Ordinary Loan Requests</a>
                            </li>
                        </ol>
                    </div>
                </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example3" class="display nowrap">
                                        ...
                                        <tbody>
                                        <tr>
                                            @foreach($ordinary_loan_requests as $ordinary_loan_request)
                                            <td>{{$ordinary_loan_request->user->firstname}} {{$ordinary_loan_request->user->lastname}}</td>
                                            <td>{{$ordinary_loan_request->created_at}}</td>
                                            <td>₦{{number_format($ordinary_loan_request->amount)}}</td>
                                            <td>{{$ordinary_loan_request->interest}}%</td>
                                            <td>{{$ordinary_loan_request->duration}}</td>
                                                <td><a href="storage/pay_slips/{{$ordinary_loan_request->user->firstname}}_{{$ordinary_loan_request->user->lastname}}/{{$ordinary_loan_request->pay_slip}}">{{$ordinary_loan_request->pay_slip}}</a></td>

                                            <td>
                                                <span class="text badge badge-primary">{{$ordinary_loan_request->status}}</span>
                                            </td>
                                            <td>
                                                <form action="{{route('approve.ordinary.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$ordinary_loan_request->id}}" name="loan_id">
                                                    <input type="hidden" value="{{$ordinary_loan_request->user_id}}" name="user_id">
                                                    <a class='mr-4'><button type="submit" class="text badge badge-success">Approve</button></a>
                                                </form>
                                                <br>
                                                <form action="{{route('refund.ordinary.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$ordinary_loan_request->id}}" name="loan_id">
                                                    <input type="hidden" value="{{$ordinary_loan_request->user_id}}" name="user_id">
                                                ...
                                                <br>
                                                <form action="{{route('delete.soft.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$ordinary_loan_request->id}}" name="loan_id">
