...
<div id="main-wrapper" class="show">
    ...
    <div class="nav-header">
        <div class="brand-logo">
            <a href=""><img class="brand-title" src="{{asset('frontend/assets/images/logo.png')}}"></a>
        </div>
...
@include('executive.sidebar')
@include('executive.notification-header')
                ...
                <div class="row page-titles mx-0">
                    <div class="col-sm-6 p-md-0">
                        <div class="welcome-text">
                            <h4 class="text-primary">Appliance Loan Requests</h4>
                        </div>
                    </div>
                    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="">Home</a></li>
                            <li class="breadcrumb-item active"><a href="">Member's Appliance Loan Requests</a>
                            </li>
                        </ol>
                    </div>
                </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example3" class="display nowrap">
                                        ...
                                        <tbody>
                                        <tr>
                                            @foreach($appliance_loan_requests as $appliance_loan_request)
                                            <td>{{$appliance_loan_request->user->firstname}} {{$appliance_loan_request->user->lastname}}</td>
                                            <td>{{$appliance_loan_request->created_at}}</td>
                                            <td>₦{{number_format($appliance_loan_request->amount)}}</td>
                                            <td>{{$appliance_loan_request->interest}}%</td>
                                            <td>{{$appliance_loan_request->duration}}</td>
                                                <td><a href="storage/pay_slips/{{$appliance_loan_request->user->firstname}}_{{$appliance_loan_request->user->lastname}}/{{$appliance_loan_request->pay_slip}}">{{$appliance_loan_request->pay_slip}}</a></td>

                                            <td>
                                                <span class="text badge badge-primary">{{$appliance_loan_request->status}}</span>
                                            </td>
                                            <td>
                                                <form action="{{route('approve.appliance.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$appliance_loan_request->id}}" name="loan_id">
                                                    <input type="hidden" value="{{$appliance_loan_request->user_id}}" name="user_id">
                                                    <a class='mr-4'><button type="submit" class="text badge badge-success">Approve</button></a>
                                                </form>
                                                <br>
                                                <form action="{{route('refund.appliance.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$appliance_loan_request->id}}" name="loan_id">
                                                    <input type="hidden" value="{{$appliance_loan_request->user_id}}" name="user_id">
                                                ...
                                                <form action="{{route('delete.appliance.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$appliance_loan_request->id}}" name="loan_id">
