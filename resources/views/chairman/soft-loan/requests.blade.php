                ...
                <div class="row page-titles mx-0">
                    <div class="col-sm-6 p-md-0">
                        <div class="welcome-text">
                            <h4 class="text-primary">Soft Loan Requests</h4>
                        </div>
                    </div>
                    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="index.html">Home</a></li>
                            <li class="breadcrumb-item active"><a href="../billing-list.html">Member's Soft Loan Requests</a>
                            </li>
                        </ol>
                    </div>
                </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example3" class="display nowrap">
                                        ...
                                        <tbody>
                                        <tr>
                                            @foreach($soft_loan_requests as $soft_loan_request)
                                            <td>{{$soft_loan_request->id}}</td>
                                            <td>{{$soft_loan_request->user->firstname}} {{$soft_loan_request->user->lastname}}</td>
                                            <td>{{$soft_loan_request->created_at}}</td>
                                            <td>₦{{number_format($soft_loan_request->amount)}}</td>
                                            <td>{{$soft_loan_request->interest}}%</td>
                                            <td>{{$soft_loan_request->duration}}</td>
                                                <td><a href="storage/pay_slips/{{$soft_loan_request->user->firstname}}_{{$soft_loan_request->user->lastname}}/{{$soft_loan_request->pay_slip}}">{{$soft_loan_request->pay_slip}}</a></td>

                                            <td>
                                                <span class="text badge badge-primary">{{$soft_loan_request->status}}</span>
                                            </td>
                                            <td>
                                                <form action="{{route('approve.soft.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$soft_loan_request->id}}" name="loan_id">
                                                    <a class='mr-4'><button type="submit" class="text badge badge-success">Approve</button></a>
                                                </form>
                                                <br>
                                                <form action="{{route('refund.soft.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$soft_loan_request->id}}" name="loan_id">
                                                    <input type="hidden" value="{{$soft_loan_request->user_id}}" name="user_id">
