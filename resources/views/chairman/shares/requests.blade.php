                ...
                <div class="row page-titles mx-0">
                    <div class="col-sm-6 p-md-0">
                        <div class="welcome-text">
                            <h4 class="text-primary">Shares Requests</h4>
                        </div>
                    </div>
                    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="">Home</a></li>
                            <li class="breadcrumb-item active"><a href="">Member's Shares Requests</a>
                            </li>
                        </ol>
                    </div>
                </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example3" class="display nowrap">
                                        ...
                                        <tbody>
                                        <tr>
                                            @foreach($shares_requests as $shares_request)
                                                <td>{{$shares_request->id}}</td>
                                                <td>{{$shares_request->user->firstname}} {{$shares_request->user->lastname}}</td>
                                                <td>{{$shares_request->created_at}}</td>
                                                <td>₦{{number_format($shares_request->amount)}}</td>
                                                <td>
                                                    <span class="text badge badge-primary">{{$shares_request->status}}</span>
                                                </td>
                                                <td>
                                                    <form action="{{route('approve-shares')}}" method="post">
                                                        @csrf
                                                        <input type="hidden" value="{{$shares_request->id}}" name="shares_id">
                                                        <input type="hidden" value="{{$shares_request->amount}}" name="amount">
