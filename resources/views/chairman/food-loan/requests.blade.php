                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example3" class="display nowrap">
                                        ...
                                        <tbody>
                                        <tr>
                                            @foreach($food_loan_requests as $food_loan_request)
                                            <td>{{$food_loan_request->id}}</td>
                                            <td>{{$food_loan_request->user->firstname}} {{$food_loan_request->user->lastname}}</td>
                                            <td>{{$food_loan_request->created_at}}</td>
                                            <td>₦{{number_format($food_loan_request->amount)}}</td>
                                            <td>{{$food_loan_request->interest}}%</td>
                                            <td>{{$food_loan_request->duration}}</td>
                                                <td><a href="storage/pay_slips/{{$food_loan_request->user->firstname}}_{{$food_loan_request->user->lastname}}/{{$food_loan_request->pay_slip}}">{{$food_loan_request->pay_slip}}</a></td>

                                            <td>
                                                <span class="text badge badge-primary">{{$food_loan_request->status}}</span>
                                            </td>
                                            <td>
                                                <form action="{{route('approve.food.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$food_loan_request->id}}" name="loan_id">
                                                    <input type="hidden" value="{{$food_loan_request->user_id}}" name="user_id">
                                                    <a class='mr-4'><button type="submit" class="text badge badge-success">Approve</button></a>
                                                </form>
                                                <br>
                                                <form action="{{route('refund.food.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$food_loan_request->id}}" name="loan_id">
                                                    <input type="hidden" value="{{$food_loan_request->user_id}}" name="user_id">
                                                ...
                                                <form action="{{route('delete.food.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$food_loan_request->id}}" name="loan_id">
