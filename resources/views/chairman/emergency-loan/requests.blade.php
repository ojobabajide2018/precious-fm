                ...
                <div class="row page-titles mx-0">
                    <div class="col-sm-6 p-md-0">
                        <div class="welcome-text">
                            <h4 class="text-primary">Emergency Loan Requests</h4>
                        </div>
                    </div>
                    <div class="col-sm-6 p-md-0 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="">Home</a></li>
                            <li class="breadcrumb-item active"><a href="">Member's Emergency Loan Requests</a>
                            </li>
                        </ol>
                    </div>
                </div>
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="example3" class="display nowrap">
                                        ...
                                        <tbody>
                                        <tr>
                                            @foreach($emergency_loan_requests as $emergency_loan_request)
                                            <td>{{$emergency_loan_request->user->firstname}} {{$emergency_loan_request->user->lastname}}</td>
                                            <td>{{$emergency_loan_request->created_at}}</td>
                                            <td>₦{{number_format($emergency_loan_request->amount)}}</td>
                                            <td>{{$emergency_loan_request->interest}}%</td>
                                            <td>{{$emergency_loan_request->duration}}</td>
                                                <td><a href="storage/pay_slips/{{$emergency_loan_request->user->firstname}}_{{$emergency_loan_request->user->lastname}}/{{$emergency_loan_request->pay_slip}}">{{$emergency_loan_request->pay_slip}}</a></td>

                                            <td>
                                                <span class="text badge badge-primary">{{$emergency_loan_request->status}}</span>
                                            </td>
                                            <td>
                                                <form action="{{route('approve.emergency.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$emergency_loan_request->id}}" name="loan_id">
                                                    <a class='mr-4'><button type="submit" class="text badge badge-success">Approve</button></a>
                                                </form>
                                                <br>
                                                <form action="{{route('refund.emergency.loan')}}" method="post">
                                                    @csrf
                                                    <input type="hidden" value="{{$emergency_loan_request->id}}" name="loan_id">
                                                    <input type="hidden" value="{{$emergency_loan_request->user_id}}" name="user_id">
